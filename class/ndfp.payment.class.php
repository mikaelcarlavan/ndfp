...
class NdfpPayment extends Paiement
{
	...
	*/
	 ...
	 * @return 	int		      <0 if KO, >0 if OK
	 */
	function create($user, $close_ndfp = 1, $thirdparty = null)
	{
		global $conf, $langs;
    ...
    function add_payment_to_bank($user)
    {
        ...
        if ($conf->banque->enabled)
        {
			...
			$user_note = new User($this->db);
            $result = $user_note->fetch($this->fk_user);
            ...
            $this->fk_bank = $acc->addline($this->datep, $this->fk_payment,  $langs->trans('UserInvoicePayment').' '.$user_note->getFullName($langs), $totalamount, $this->payment_number, '', $user, '', '');
    ...
    function get_clickable_name($withpicto = 0, $option = '', $max = 0, $short = 0)
    {
        ...
        $url = dol_buildpath('/ndfp/payment.php', 1).'?id='.$this->id;
