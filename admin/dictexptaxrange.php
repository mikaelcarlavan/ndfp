...
if ($add || $modify)
{
    ...
    $sql  = " SELECT r.rowid";
    $sql .= " FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range r";
    ...
    if (!$error)
    {
        ...
        {
            $sql  = " UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range SET `fk_cat` = ".$db->escape($fk_cat).", `range` = ".$db->escape($range).",";
            ...
            if ($result)
            {
                ...
                $sql  = " UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax SET `fk_cat` = ".$db->escape($fk_cat)."";
...
{
    $sql = "UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range SET `active` = 1 WHERE rowid = ".$rowid;
    dol_syslog("Dictexptaxrange sql=".$sql);
...
{
    $sql = "UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range SET `active` = 0 WHERE rowid = ".$rowid;
    dol_syslog("Dictexptaxrange sql=".$sql);
...
if ($action == 'confirm_delete' && $confirm == 'yes')       // delete
{
    ...
    // Check if not a 0 range
     // Check if range does not exist
    $sql  = " SELECT r.range FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range r WHERE r.rowid = ".$rowid;
    $result = $db->query($sql);
    if (! $result)
    {
        $num = $db->num_rows($result);
    ...
    $sql = "DELETE FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range WHERE rowid = ".$rowid;
    ...
    if (!$error)
    {
        $sql = " DELETE FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax WHERE fk_range = ".$rowid;
