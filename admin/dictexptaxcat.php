...
if ($add || $modify)
{
    ...
    if (!$error)
    {
        ...
        if ($add)
        {
            $sql = " INSERT INTO ".MAIN_DB_PREFIX."c_ndfp_exp_tax_cat (`fk_parent`, `label`, `active`)";
            ...
            if ($result)
            {
                $catid = $db->last_insert_id(MAIN_DB_PREFIX."c_ndfp_exp_tax_cat");

                $sql = " INSERT INTO ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range (`fk_cat`, `range`, `active`)";
                ...
                if ($result)
                {
                    $rangeid = $db->last_insert_id(MAIN_DB_PREFIX."c_ndfp_exp_tax_range");

                    $sql = " INSERT INTO ".MAIN_DB_PREFIX."c_ndfp_exp_tax (`fk_cat`, `fk_range`, `coef`, `offset`)";
        ...
        {
            $sql  = " UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax_cat SET `fk_parent` = '".$db->escape($fk_parent)."', `label` = '".$db->escape($label)."',";
...
if ($action == $acts[0])
{       // activate
    $sql = "UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax_cat SET `active` = 1 WHERE rowid = ".$rowid;
...
if ($action == $acts[1])
{       // disable
    $sql = "UPDATE ".MAIN_DB_PREFIX."c_ndfp_exp_tax_cat SET `active` = 0 WHERE rowid = ".$rowid;
...
{       // delete
    ...
    $sql = "DELETE FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax_cat WHERE rowid = ".$rowid;
    ...
    $sql = " DELETE FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax_range WHERE fk_cat = ".$rowid;
    ...
    $sql = " DELETE FROM ".MAIN_DB_PREFIX."c_ndfp_exp_tax WHERE fk_cat = ".$rowid;
